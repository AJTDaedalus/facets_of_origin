...
> **Naming note (2026-08-09):** the name "Resonance" proposed above has since been used for a Soul *domain* (sound, vibration, harmonic force — PHB II.3) and must not be reused as a tradition name. The core PHB deliberately names no traditions: it keys them to attributes instead — Spirit governs intuitive magic, Knowledge governs scholarly magic (II.3, *Rolling Magic*), and as of T4.1/D7 each keys a skill as well (Spirit adds Attune, Knowledge adds Lore). **Settled (owner rulings, 2026-08-09, DECISIONS D15 and D16):** the setting-layer tradition names are **Invocation** (intuitive — Spirit/Attune, Soul-aligned), **Thaumaturgy** (scholarly — Knowledge/Lore, Mind-aligned), and **Tempering** (Body-aligned). The "Wild" tradition proposed above is dead: that territory went to Soul (Verdance, The Living World, Beasts), so a nature-flavoured Body name would point at another Facet's domains. A Tempering practitioner is **a Tempered** (collectively *the Tempered*), alongside an Invoker and a Thaumaturge. Still open: Tempering's casting attribute and skill — see D16.
