...
# Roster of characters and attributes
PLAYERS = {
    "Serane": {
...
def start_server():
    ...
    # Spawn the uvicorn process running run.py
...
def main():
    ...
    try:
        ...
        for idx in range(1, 11):
            mm_name = "Arthur" if idx <= 5 else "Cyrus"
            ...
            # Collect rolls for this session
            rolls_in_session = []
            
            # We simulate different actions depending on the session style
            if idx in (1, 6): # Arrival gates
                # Roll Persuade/Attune
                for p_name, ws in player_ws_list.items():
                    ws.send(json.dumps({
                        "type": "roll",
                        "attribute_id": "charisma" if p_name != "Dassa" else "strength",
                        "skill_id": "persuade" if p_name != "Dassa" else "combat",
                        "difficulty": "Standard",
                        "sparks_spent": 1 if idx == 6 else 0
                    }))
                    time.sleep(0.1)
            ...
            elif idx in (2, 7): # Social Agendas
                for p_name, ws in player_ws_list.items():
                    ws.send(json.dumps({
                        "type": "roll",
                        "attribute_id": "wisdom",
                        "skill_id": "persuade",
                        "difficulty": "Hard" if idx == 2 else "Standard",
                        "sparks_spent": 0
                    }))
                    time.sleep(0.1)
                    res = read_until(ws, "roll_result")
                ...
                for p_name, ws in player_ws_list.items():
                    ws.send(json.dumps({
                        ...
                        "difficulty": "Hard",
                        "sparks_spent": 1 if idx == 8 else 0
                    }))
                    time.sleep(0.1)
                    res = read_until(ws, "roll_result")
                    if res:
                        ...
                        rolls_in_session.append({
                            "player": p_name,
                            "action": "summons_check",
                            "total": roll_info["total"],
                            "outcome": roll_info["outcome"]
                        })
                        
            elif idx in (4, 9): # Midnight Ballroom Combat
                mm_ws.send(json.dumps({"type": "combat_start"}))
                time.sleep(0.1)
                
                # Declare postures
                player_ws_list["Dassa"].send(json.dumps({"type": "declare_posture", "posture": "defensive"}))
                player_ws_list["Serane"].send(json.dumps({"type": "declare_posture", "posture": "measured"}))
                player_ws_list["Pello"].send(json.dumps({"type": "declare_posture", "posture": "aggressive" if idx == 9 else "measured"}))
                player_ws_list["Ilesse"].send(json.dumps({"type": "declare_posture", "posture": "withdrawn" if idx == 9 else "measured"}))
                time.sleep(0.1)
                
                mm_ws.send(json.dumps({"type": "reveal_postures"}))
                time.sleep(0.1)
                
                mm_ws.send(json.dumps({"type": "spawn_enemy", "enemy_id": "the_hollow", "instance_name": "The Hollow"}))
                time.sleep(0.1)
                
                # Combat exchange strikes
                for p_name, ws in player_ws_list.items():
                    if p_name in ("Dassa", "Pello"):
                        ws.send(json.dumps({
                            ...
                            "target": "The Hollow",
                            "difficulty": "Standard",
                            "sparks_spent": 1 if idx == 9 else 0
                        }))
                        time.sleep(0.1)
            ...
            elif idx in (5, 10): # East Wing Escape
                mm_ws.send(json.dumps({"type": "combat_start"}))
                time.sleep(0.1)
                
                # Serane casts spell
                player_ws_list["Serane"].send(json.dumps({
                    "type": "cast",
                    "scope": "minor",
                    "intent": "Weave a crystal shield to block the corridor from the Radiant's fire"
                }))
                time.sleep(0.1)
                res = read_until(player_ws_list["Serane"], "cast_result")
                if res:
                    roll_info = res["roll"]
                    rolls_in_session.append({
                        "player": "Serane",
                ...
                # Ilesse casts spell
                player_ws_list["Ilesse"].send(json.dumps({
                    "type": "cast",
                    "scope": "minor",
                    "intent": "Project a resonant scream to shatter the wight's focus"
                }))
                time.sleep(0.1)
                res = read_until(player_ws_list["Ilesse"], "cast_result")
                if res:
                    roll_info = res["roll"]
                    rolls_in_session.append({
                        "player": "Ilesse",
                        "action": "cast_resonance",
                        "total": roll_info["total"],
                        "outcome": roll_info["outcome"]
                    })
        ...
        print(f"Wrote batch_results.json and dice_rolls.txt to {PLAYTEST_DIR}")
